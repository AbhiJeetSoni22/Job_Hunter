...
- **Authentication & Multi-User Support**: User registration, JWT sessions, and multi-tenant database isolation.
...
- **Single User Scope**: The system assumes exactly one active resume and single-user local deployment.
- **Database Dependency for Tests**: 110 of 123 backend pytest tests require a live PostgreSQL database configured via `TEST_DATABASE_URL` (13 non-database unit tests pass without a DB).
