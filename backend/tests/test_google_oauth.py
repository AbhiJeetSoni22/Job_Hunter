...
def test_callback_missing_state_rejects(client: TestClient):
...
def test_callback_mismatched_state_rejects(client: TestClient):
...
def test_callback_missing_verifier_rejects(client: TestClient):
...
def test_callback_google_error_param_redirects(client: TestClient):
...
def test_handoff_store_single_use_and_replay_prevention():
...
def test_handoff_store_expired_code_rejected():
...
def test_exchange_endpoint_rejects_invalid_code(client: TestClient):
...
@needs_db
def test_google_only_user_cannot_login_with_password(client: TestClient, db):
